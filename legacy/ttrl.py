...
from utils import run_rl
from utils import variants_to_parquet
...
def main(
    ...
    for q_id, question_data in enumerate(data):
        variants_to_parquet(
            data=question_data, output_path=parquet_output_path, question_id=q_id
        )
        print(
            f"Saved variants for question {q_id} to"
            f" {parquet_output_path}/variants_q{q_id}.parquet and"
            f" {parquet_output_path}/variants_q{q_id}.json"
        )
        print(
            f"Saved val test of question pass@16 {q_id} to"
            f" {parquet_output_path}/test_q{q_id}.parquet and"
            f" {parquet_output_path}/test_q{q_id}.json"
        )
    ...
    for q_id, question_data in enumerate(data):
        ...
        last_checkpoint_path = run_rl(
            model_dir=model_dir,
            train_parquet_path=f"{parquet_output_path}/variants_q{q_id}.parquet",
            test_parquet_path=f"{parquet_output_path}/test_q{q_id}.parquet",
        ...
        instruction_following = (
            ...
            " constants of integration. DO NOT OUTPUT IN LATEX FORMAT. OUTPUT IN SYMPY"
            " in <answer> tags."
        )
        ...
        for i in range(max(passes)):
            base_response = run_inference(
                model_dir=model_dir, prompt=prompt, max_new_tokens=max_new_tokens
            )
            base_responses.append(base_response)
            print(f"Base Model Response {i+1}: {base_response}")
            is_correct = verify_response(
        ...
        for i in range(max(passes)):
            rl_response = run_inference(
                model_dir=last_checkpoint_path,
                prompt=prompt,
                max_new_tokens=max_new_tokens,
            )
    ...
    base_pass_k_correct = {
        p: sum(1 for r in results if r["base_model"][f"pass@{p}"]) for p in passes
    }
    rl_pass_k_correct = {
        p: sum(1 for r in results if r["rl_model"][f"pass@{p}"]) for p in passes
    }
    ...
    with open(
        os.path.join(
            os.path.dirname(f"/home/ubuntu/test/TTRL/checkpoints/{project_name}"),
            "results.json",
        ),
        "w",
    ) as f:
